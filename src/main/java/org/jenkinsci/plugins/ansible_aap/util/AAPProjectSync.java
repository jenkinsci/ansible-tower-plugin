...
public class AAPProjectSync implements Serializable {
    private AAPConnector connection = null;
    private AAPProject projectReference = null;
    private JSONObject syncData = null;
    private long lastLogId = 0L;

    public AAPProjectSync(AAPConnector connection, AAPProject projectReference) throws AnsibleAAPException {
        ...
        if (response.getStatusLine().getStatusCode() != 202 && response.getStatusLine().getStatusCode() != 200) {
            throw new AnsibleAAPException("Unexpected error code returned when launching project sync (" + response.getStatusLine().getStatusCode() + ")");
        }
        try {
            String json = EntityUtils.toString(response.getEntity());
            this.syncData = JSONObject.fromObject(json);
        } catch (IOException ioe) {
            throw new AnsibleAAPException("Unable to read project sync launch response and convert it into json: " + ioe.getMessage());
        }
    }

    private void loadSync(int method)  throws AnsibleAAPException {
        HttpResponse response = this.connection.makeRequest(method, syncData.getString("url"), null, false);
        if (response.getStatusLine().getStatusCode() != 200) {
            throw new AnsibleAAPException("Unexpected error code returned when loading project sync (" + response.getStatusLine().getStatusCode() + ")");
        }
        try {
            String json = EntityUtils.toString(response.getEntity());
            this.syncData = JSONObject.fromObject(json);
        } catch (IOException ioe) {
            throw new AnsibleAAPException("Unable to read project sync response and convert it into json: " + ioe.getMessage());
        }
    }
    ...
    public boolean wasSuccessful() throws AnsibleAAPException {
        ...
        throw new AnsibleAAPException("Did not get a failed status from project sync!");
    }
        ...
        while(keepChecking) {
            ...
            if (response.getStatusLine().getStatusCode() == 200) {
                ...
                try {
                    ...
                    throw new AnsibleAAPException("Unable to read project sync event response and convert it into json: " + ioe.getMessage());
                }
                ...
                throw new AnsibleAAPException("Unexpected error code returned when getting project sync events (" + response.getStatusLine().getStatusCode() + ")");
            }
        }
        return events;
    }

    public void cancelSync() throws AnsibleAAPException {
        ...
        if (response.getStatusLine().getStatusCode() != 202 && response.getStatusLine().getStatusCode() != 405) {
            throw new AnsibleAAPException("Unexpected response code from project sync cancel ("+ response.getStatusLine().getStatusCode() +")");
        }
    }
